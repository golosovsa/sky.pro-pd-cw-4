...
class TestFavoriteMoviesDAO:
    ...
    def favorite_movies_dao(self, db):
        return FavoriteMovieDAO(db.session)

    @pytest.fixture()
    def favorite_movie_1(self, db):
        model = FavoriteMovies(user_id=1, movie_id=2)
    ...
    def favorite_movie_2(self, db):
        model = FavoriteMovies(user_id=1, movie_id=1)
        db.session.add(model)
        db.session.commit()
        return model
    ...
    def favorite_movie_3(self, db):
        model = FavoriteMovies(user_id=2, movie_id=3)
        db.session.add(model)
        db.session.commit()
        return model
    ...
    def favorite_movie_4(self, db):
        model = FavoriteMovies(user_id=2, movie_id=1)
        db.session.add(model)
        db.session.commit()
        return model
    ...
    def test_favorite_movie_create(self, favorite_movies_dao):
        favorite_movie = FavoriteMovies(user_id=99, movie_id=99)
